...
func (s *Server) handlePairApprove(w http.ResponseWriter, r *http.Request) {
	...
	s.auditEvent(r, "devices.pair_approved", acc.ID, "", "approved device pairing PIN")
...
func (s *Server) handlePairRedeem(w http.ResponseWriter, r *http.Request) {
	...
	s.auditEvent(r, "devices.paired", sess.UserID, dev.ID, "completed device pairing for: "+dev.DeviceName)
...
func (s *Server) handleRevokeDevice(w http.ResponseWriter, r *http.Request) {
	...
	s.auditEvent(r, "devices.revoked", acc.ID, deviceID, "revoked trusted device")
	writeJSON(w, http.StatusOK, map[string]bool{"ok": true})
}
